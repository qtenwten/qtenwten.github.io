...
function NumberToWords() {
  ...
  const buildWordsWithMinor = (num, curr, options = {}) => {
    ...
    const currencyWord = getCurrencyWord(integerValue, curr, language)
    ...
    if (forceMinor) {
      if (digitMinor) {
        resultText = `${textWithoutCurrency} ${currencyWord} ${minorDigits} ${pluralizeMinor(Number(minorDigits), curr, language)}`
      } else {
        const fullText = numberToWords(normalized, curr, true, 'none', 0, language).text
        resultText = Number(minorDigits) === 0
          ? `${baseWithoutMinor} ${language === 'en' ? 'zero' : 'ноль'} ${pluralizeMinor(0, curr, language)}`
          : fullText
      }
    }
              ...
              <ResultSection tone="success" className="number-to-words-primary-card">
                ...
                <div className="number-to-words-result-block">
                  ...
                  <div className="number-to-words-result-actions">
                    <CopyButton text={primaryVariant.text} className="number-to-words-copy number-to-words-main-copy" analytics={{ toolSlug: 'number-to-words', linkType: 'result' }} />
                  </div>
                </div>
              </ResultSection>
            )}
